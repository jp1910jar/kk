...
import {
  Star, Heart, Share2, ShoppingBag, Truck, ShieldCheck, RotateCcw, ChevronRight, Check, Download, ZoomIn,
} from "lucide-react";
...
export const Route = createFileRoute("/product/$id")({
  ...
  notFoundComponent: () => (
    <div className="container-px mx-auto max-w-7xl py-40 text-center">
      ...
      <Link to="/products"><Button variant="accent" className="mt-6">Back to products</Button></Link>
    </div>
  ),
});
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <div>
            ...
            >
              <img
                src={gallery[active]}
                alt={product.name}
                className={cn("h-full w-full object-cover transition-transform duration-500", zoom && "scale-150")}
              />
          ...
          <div>
            <div className="lg:sticky lg:top-24">
              <span className="text-xs uppercase tracking-wide text-muted-foreground">{product.brand} · {product.category}</span>
              <h2 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">{product.name}</h2>
              <div className="mt-3 flex items-center gap-3 text-sm">
                <span className="inline-flex items-center gap-1">
                  <Star className="h-4 w-4 fill-accent text-accent" /> {product.rating}
                </span>
                <span className="text-muted-foreground">{product.reviews} reviews</span>
                {product.inStock && <span className="inline-flex items-center gap-1 text-accent"><Check className="h-4 w-4" /> In stock</span>}
              </div>

              <div className="mt-5 flex items-end gap-3">
                <span className="text-3xl font-bold">{formatINR(product.price)}</span>
                <span className="mb-1 text-lg text-muted-foreground line-through">{formatINR(product.mrp)}</span>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Button size="lg" onClick={() => { addToCart(product.id); navigate({ to: "/checkout" }); }}>
                  Buy now
                </Button>
                <Button variant="outline" size="icon" onClick={() => toggleWishlist(product.id)} aria-label="Wishlist">
                  <Heart className={cn("h-5 w-5", saved && "fill-accent text-accent")} />
                </Button>
                <Button variant="outline" size="icon" aria-label="Share"><Share2 className="h-5 w-5" /></Button>
              </div>
        ...
        <div className="mt-16">
          <Tabs defaultValue="specs">
            ...
            <TabsContent value="features" className="mt-6">
              <ul className="grid gap-3 sm:grid-cols-2">
                {product.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 rounded-2xl border border-border bg-card p-4">
                    <Check className="mt-0.5 h-4 w-4 shrink-0 text-accent" /> {f}
                  </li>
                ))}
              </ul>
            ...
            <TabsContent value="warranty" className="mt-6">
              <div className="rounded-2xl border border-border bg-card p-6 text-muted-foreground">{product.warranty}</div>
            </TabsContent>
            <TabsContent value="downloads" className="mt-6">
              <div className="space-y-3">
                {["User Manual (PDF)", "Quick Start Guide", "Warranty Card"].map((d) => (
                  <a key={d} href="#" className="flex items-center justify-between rounded-2xl border border-border bg-card p-4 hover:border-accent">
                    <span>{d}</span><Download className="h-4 w-4 text-accent" />
                  </a>
                ))}
              </div>
            </TabsContent>
